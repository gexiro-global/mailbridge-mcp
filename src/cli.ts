...
const program = new Command()
  ...
  .option("--config <path>", "configuration file", process.env.MAILBRIDGE_CONFIG ?? "./config/mailboxes.example.yaml");
...
program.command("doctor").description("check config and required secret-file presence without reading values").action(async () => {
  ...
  const references = new Set<string>([process.env.MAILBRIDGE_ID_KEY_FILE ?? "mailbridge_id_hmac_key"]);
  for (const entry of config.mailboxes.filter((item) => item.enabled)) {
    references.add(entry.username_secret);
    references.add(entry.password_secret);
  }
  for (const reference of references) checks.push({ reference, present: await secrets.exists(reference) });
  output({ config_valid: true, secret_checks: checks, ready: checks.every((check) => check.present) });
